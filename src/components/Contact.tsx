      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto mb-8">
            Ready to accelerate your business growth? Let's discuss how our digital marketing expertise can help you achieve your goals.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                {isSubmitted ? (
                  ...
                  >
                    ...
                    <p className="text-gray-300 mb-6">
                      Thank you for reaching out! We'll review your information and get back to you within 24 hours with a customized strategy proposal.
                    </p>
                    <Badge className="bg-gradient-to-r from-purple-600 to-teal-500 text-white px-4 py-2">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <motion.div variants={itemVariants} className="space-y-2">
                      ...
                      <Select value={formData.service} onValueChange={(value) => handleInputChange('service', value)}>
                        <SelectTrigger className="bg-slate-800/50 border-slate-700 focus:border-purple-500 text-white">
                          <SelectValue placeholder="Select a service" />
                        </SelectTrigger>
                        <SelectContent className="bg-slate-800 border-slate-700">
                          {services.map((service) => (
                            <SelectItem key={service} value={service} className="text-white focus:bg-purple-600/20">
                              {service}
                            </SelectItem>
                          ))}
                        </SelectContent>
                        ...
                        <Select value={formData.budget} onValueChange={(value) => handleInputChange('budget', value)}>
                          ...
                          <SelectContent className="bg-slate-800 border-slate-700">
                            {budgetRanges.map((range) => (
                              <SelectItem key={range} value={range} className="text-white focus:bg-purple-600/20">
                                {range}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        ...
                        <Select value={formData.urgency} onValueChange={(value) => handleInputChange('urgency', value)}>
                          ...
                          <SelectContent className="bg-slate-800 border-slate-700">
                            {urgencyOptions.map((option) => (
                              <SelectItem key={option} value={option} className="text-white focus:bg-purple-600/20">
                                {option}
                              </SelectItem>
                            ))}
                          </SelectContent>
                    ...
                    >
                      ...
                      >
                        ...
                        {isSubmitting ? 'Sending Message...' : 'Send Message & Get Free Strategy Call'}
                      </Button>
                    </motion.div>
                  </form>
                )}
          ...
          >
                    ...
                    <CardHeader className="pb-3">
                      <CardTitle className="text-lg text-white flex items-center">
                        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${
                          info.color === 'purple' ? 'from-purple-500/20 to-purple-600/20' : 'from-teal-500/20 to-teal-600/20'
                        } flex items-center justify-center mr-3`}>
                          <Icon className={`w-5 h-5 ${
                            info.color === 'purple' ? 'text-purple-400' : 'text-teal-400'
                          }`} />
                        </div>
                        {info.title}
                      </CardTitle>
            ...
            <motion.div variants={itemVariants} className="space-y-4">
              <Button className="w-full bg-gradient-to-r from-purple-600 to-teal-500 hover:from-purple-700 hover:to-teal-600 text-white py-4 font-semibold">
                <Calendar className="mr-2 w-5 h-5" />
                Schedule a Call
              </Button>
              
              <Button variant="outline" className="w-full border-purple-500/50 text-purple-400 hover:bg-purple-500/10 py-4 font-semibold">
                <Zap className="mr-2 w-5 h-5" />
                Get Instant Quote
              </Button>
              
              <Button variant="outline" className="w-full border-teal-500/50 text-teal-400 hover:bg-teal-500/10 py-4 font-semibold">
            ...
            <motion.div variants={itemVariants}>
              <Card className="bg-gradient-to-br from-slate-800/30 to-slate-900/30 backdrop-blur-sm border-slate-700 text-center">
                <CardContent className="pt-6">
                  <div className="text-3xl font-bold text-white mb-2">500+</div>
                  <p className="text-sm text-gray-400 mb-4">Successful Projects</p>
                  <div className="flex justify-center items-center space-x-1 mb-2">
                    {[1,2,3,4,5].map((star) => (
                      <motion.div
                        key={star}
                        animate={{ rotate: [0, 360] }}
                        transition={{ duration: 2, delay: star * 0.1, repeat: Infinity, repeatDelay: 5 }}
                      >
                        ⭐
                      </motion.div>
                    ))}
                  </div>
                  <p className="text-xs text-gray-400">4.9/5 Client Satisfaction</p>
                </CardContent>
              </Card>
            </motion.div>
          </motion.div>
          ...
          <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700 overflow-hidden">
            <div className="h-64 bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center">
              <div className="text-center">
                ...
                <Button className="mt-4 bg-gradient-to-r from-purple-600 to-teal-500 hover:from-purple-700 hover:to-teal-600 text-white">
                  View on Google Maps
                </Button>
              </div>
            </div>
